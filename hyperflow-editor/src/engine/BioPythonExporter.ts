...
// --- Types ---
interface EnzymeData { enzyme?: string; mode?: string }
interface LigaseData { circular?: boolean }
interface CrisprData { guideRNA?: string; pam?: string }
...
const sortBioNodes = (nodes: Node[]): BioOp[] => {
  ...
  return sorted.map(node => {
    ...
    let data: EnzymeData | LigaseData | CrisprData | undefined = undefined;
...
// --- Generator Function ---
export const generateBioPythonCode = (nodes: Node[]): string => {
  const ops = sortBioNodes(nodes);
  ...
  // Pipeline Logic
  const dnaVar = 'dna_seq';
  const rnaVar = 'rna_seq';
  const proteinVar = 'protein_seq';
  const fragsVar = 'fragments';
  const ligatedVar = 'ligated_seq';
  ...
  ops.forEach(op => {
    if (op.type === 'sequence') {
      ...
      if (hasDNA && dnaValid) {
        const enzymeName = (op.data as EnzymeData).enzyme;
      ...
      if (hasFragments) {
        code += `    # Node: Ligase\n`;
        if (op.isValid) {
          const isCircular = (op.data as LigaseData).circular;
      ...
      if (hasDNA && dnaValid) {
        const gRNA = (op.data as CrisprData).guideRNA || '';
        const pam = (op.data as CrisprData).pam || 'NGG';
